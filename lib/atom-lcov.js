...
function lcovDataFromLcovFile (lcovFile) {
  return (lcovFile
...
export default {
  activate (state) {
    this.lcovFile = new Rx.Subject()

    // An observable representing the latest coverage.
    const coverage = lcovDataFromLcovFile(this.lcovFile).map(parseLcov).shareReplay(1)
    ...
    this.subscriptions.add(atom.workspace.observeTextEditors(textEditor => {
      ...
      const subscription = coverage.subscribe(coverage => {
        const editorPath = textEditor.getPath()
        const fileCoverage = coverage[Object.keys(coverage).find(e => editorPath.endsWith(e))]
